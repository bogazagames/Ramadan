...
// ============================================================
// BACKFILL — تعويض أول يومين (إذا بدأنا من اليوم 3+)
// ============================================================
function addPointsToProfile(p, amount){
  if(!p) return;
  if(amount>0){
...
function _logHasAnyProgress(log){
  if(!log) return false;
  return !!(
    log.targetReached || log.maghribReached ||
    (log.deeds&&log.deeds.length) || (log.pendingDeeds&&log.pendingDeeds.length) ||
    (log.quests&&log.quests.length) || log.quizDone ||
    (log.badDeeds&&log.badDeeds.length) || (log.pts||0)!==0 || log.ateReason
  );
}

function backfillFirstTwoDaysForProfile(p){
  const td=todayTT(); // uses STATE.timetable
  if(!p || !td) return false;
  if((td.ramadanDay||0) < 3) return false;

  // pick day 1 & 2 from timetable (fallback safe)
  const tt=(STATE.timetable&&STATE.timetable.length)?STATE.timetable:FALLBACK_TT;
  const d1=tt.find(x=>x.ramadanDay===1) || tt[0];
  const d2=tt.find(x=>x.ramadanDay===2) || tt[1];
  const days=[d1,d2].filter(Boolean);

  let changed=false;
  const sumDeeds = GOOD_DEEDS.reduce((a,d)=>a+(d.pts||0),0);
  const sumQuests = QUESTS.reduce((a,q)=>a+(q.pts||0),0);
  const quizPts = 100; // 10 أسئلة × 10 نقاط (نفس منطق المسابقة)

  days.forEach(tday=>{
    const dateISO=tday.dateISO;
    const log=dayLog(p,dateISO);

    // لا تلمس اليوم إذا الطفل سجل فيه أي شيء (حتى لا نخرب بياناته)
    if(_logHasAnyProgress(log)) return;

    // 1) نجاح الوصول للهدف
    log.targetReached=true;
    const target=getTarget(p,tday);
    let ptsTarget = target.type==='maghrib'?150:100;
    if((tday.ramadanDay||0)%7===0) ptsTarget = Math.round(ptsTarget*2);

    // 2) كل الأعمال اليومية والمهام
    log.deeds = GOOD_DEEDS.map(d=>d.id);
    log.pendingDeeds = [];
    log.quests = QUESTS.map(q=>q.id);

    // 3) مسابقة اليوم (كمكتملة)
    log.quizDone = true;
    log.quizScore = quizPts;

    const total = ptsTarget + sumDeeds + sumQuests + quizPts;
    log.pts = (log.pts||0) + total;

    addPointsToProfile(p, total);
    addActivityLog(p,`🎁 تعويض اليوم ${tday.ramadanDay} (لم يفُت)`, total,'approved');
    changed=true;
  });

  if(changed){
    p.backfillRamadan12 = true;
    checkAch(p);
  }
  return changed;
}

function backfillFirstTwoDaysForAll(){
  let any=false;
  (STATE.profiles||[]).forEach(p=>{
    if(backfillFirstTwoDaysForProfile(p)) any=true;
  });
  if(any) save();
}
...
async function createProfile(){
  ...
  // إذا بدأنا من اليوم 3+ نعوض الطفل افتراضياً عن أول يومين
  backfillFirstTwoDaysForProfile(p);
  save();
...
function renderStatusCard(p,log,td){
  ...
  else if(log.ateReason && log.ateReason!=='forgot'){
...
function confirmAte(reason){
  ...
  const effects={
    ...
    forgot:{pts:0,penalty:false,msg:'✅ تم تسجيل النسيان فقط — كمل صيامك عادي 😊'},
...
function renderQuizPath(p){
  ...
  <style>
    ...
    .map-header-sub{font-size:.9rem;font-weight:800;color:rgba(240,230,211,.78);margin-top:0}
    ...
    .map-hint{text-align:center;font-size:0.82rem;color:var(--text);padding:8px 12px;font-weight:800;background:rgba(240,180,41,0.08);border-radius:10px;margin-bottom:6px;border:1px solid rgba(240,180,41,0.2)}
